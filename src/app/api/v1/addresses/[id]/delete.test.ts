...
describe('DELETE /api/v1/addresses/:id', () => {
  describe('Anonymous user', () => {
    test('Delete address', async () => {
      const response = await fetch('http://localhost:3000/api/v1/addresses/1', {method: 'DELETE'});
    ...
    test('Delete address not found', async () => {
      const response = await fetch('http://localhost:3000/api/v1/addresses/88', {method: 'DELETE'});
